	...
	namespace general_parser
	{
			...
			const vector<shared_ptr<GeneralTokenDefine>> &tokens,const unordered_map<wstring, GeneralTokenDefine::TokenOptional>& infos)
		{
			...
			while (textBegin < content.end())
			{
				...
				{
					stream.emplace_back(make_shared<TokenInfo>(str, name, findIter->first - content.begin(), findIter->length()));
				}
				textBegin = findIter->second;
			}
		...
		vector<shared_ptr<TokenInfo>> GeneralLRExecutor::ParseToken(const wstring& fileName)
		{
